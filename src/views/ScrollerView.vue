<template>
  <scroller class="list" append="tree">
    ...
    <refresh class="refresh-view" :display="refresh_display" @refresh="onrefresh">
      ...
      <text class="refresh-arrow" style="text-align: center; color:rgb(238, 162, 54)">{{refreshText}}</text>
    </refresh>

    <div class="section" v-for="sec in sections">
      <div class="header">
        <text class="header-title">{{sec.title}}</text>
      </div>
      <div class="item" v-for="item in sec.items">
...
<script>
  ...
  export default {
    computed: {
      refreshText () {
        return this.refresh_display === 'hide' ? '下拉刷新' : '正在玩命加载...'
      },
      loadingText () {
        return this.loading_display === 'hide' ? '上拉加载' : '正在玩命加载...'
      }
    },
    methods: {
      onrefresh: function (e) {
        ...
        setTimeout(() => { self.refresh_display = 'hide' }, 3000)
      },
